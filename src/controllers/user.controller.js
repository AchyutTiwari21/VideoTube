...
const updateUserAvatar = asyncHandler(async(req, res) => {
    ...
    if(user) {
        const response = await deleteFromCloudinary(req.user.avatar);
        if(!response) throw new ApiError(500, "Error while deleting the file.");
    }
...
const updateUserCoverImage = asyncHandler(async(req, res) => {
    ...
    if(user) {
        const response = await deleteFromCloudinary(req.user.coverImage);
        if(!response) throw new ApiError(500, "Error while deleting the file.");
    }
